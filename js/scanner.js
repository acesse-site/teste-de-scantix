    ...
    <div class="product-result-card">
      <div class="product-result-top">
        ${hasImage
          ...
          : `<div class="product-thumb-placeholder" style="cursor:default">
        ...
        <div class="product-info">
          ...
          ${!hasImage
            ? `<button class="btn-primary" style="margin-top:.375rem;height:2rem;font-size:.8125rem;padding:0 .875rem;border-radius:.5rem;display:inline-flex;align-items:center;gap:.375rem" onclick="openScannerPhotoModal()">
...
// ===== MODAL DE FOTO DO SCANNER =====
let _scannerPhotoFile = null;
let _scannerPhotoUrl  = '';

function openScannerPhotoModal() {
  _scannerPhotoFile = null;
  _scannerPhotoUrl  = '';
  // Reset estado do modal
  const wrap    = document.getElementById('scanner-photo-preview-wrap');
  const preview = document.getElementById('scanner-photo-preview');
  const urlInput= document.getElementById('scanner-photo-url');
  const status  = document.getElementById('scanner-photo-status');
  if (wrap)    wrap.classList.add('hidden');
  if (preview) preview.src = '';
  if (urlInput)urlInput.value = '';
  if (status)  { status.classList.add('hidden'); status.textContent = ''; }
  const btn = document.getElementById('scanner-photo-save-btn');
  if (btn) { btn.disabled = false; btn.innerHTML = '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M19 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11l5 5v11a2 2 0 0 1-2 2z"/><polyline points="17 21 17 13 7 13 7 21"/><polyline points="7 3 7 8 15 8"/></svg> Salvar foto'; }
  document.getElementById('scanner-photo-modal-overlay').classList.remove('hidden');
  document.getElementById('scanner-photo-modal').classList.remove('hidden');
}

function closeScannerPhotoModal() {
  document.getElementById('scanner-photo-modal-overlay').classList.add('hidden');
  document.getElementById('scanner-photo-modal').classList.add('hidden');
  _scannerPhotoFile = null;
  _scannerPhotoUrl  = '';
}

// toggleScannerPhotoPanel mantido como alias para não quebrar chamadas antigas
function toggleScannerPhotoPanel() { openScannerPhotoModal(); }
...
function handleScannerPhotoSelect(event) {
  const file = event.target.files?.[0];
  if (!file) return;
  if (!file.type.startsWith('image/')) {
    setScannerPhotoStatus('Selecione uma imagem válida.', 'error');
    return;
  }
  _scannerPhotoFile = file;
  _scannerPhotoUrl  = '';
  const reader = new FileReader();
  reader.onload = e => {
    const wrap = document.getElementById('scanner-photo-preview-wrap');
    ...
    if (urlInput) urlInput.value = '';
    setScannerPhotoStatus('');
  };
  reader.readAsDataURL(file);
}

function previewScannerPhotoUrl(url) {
  _scannerPhotoFile = null;
  _scannerPhotoUrl  = url.trim();
  const wrap = document.getElementById('scanner-photo-preview-wrap');
  const preview = document.getElementById('scanner-photo-preview');
  if (!url.trim()) {
    if (wrap) wrap.classList.add('hidden');
  ...
  if (preview) {
    preview.src = url.trim();
    preview.onload  = () => { if (wrap) wrap.classList.remove('hidden'); setScannerPhotoStatus(''); };
    preview.onerror = () => { if (wrap) wrap.classList.add('hidden'); setScannerPhotoStatus('URL inválida ou imagem inacessível.', 'error'); };
  }
}

function setScannerPhotoStatus(msg, type = 'info') {
  const el = document.getElementById('scanner-photo-status');
  if (!el) return;
  if (!msg) { el.classList.add('hidden'); el.textContent = ''; return; }
  el.textContent = msg;
  el.style.color = type === 'error' ? 'var(--destructive)' : 'hsl(160,60%,35%)';
  el.classList.remove('hidden');
}

async function saveScannerPhoto() {
  if (!scannerProduct) return;
  if (!_scannerPhotoFile && !_scannerPhotoUrl) {
    setScannerPhotoStatus('Escolha uma foto ou cole uma URL primeiro.', 'error');
    return;
  }
  ...
  btn.disabled = true;
  btn.innerHTML = '<span class="spinner"></span>';
  setScannerPhotoStatus('Salvando...');

  try {
    let imageUrl = '';

    if (_scannerPhotoFile) {
      // Upload para o Storage
      const ean = scannerProduct.ean;
      const path = `products/${ean}_${Date.now()}.jpg`;
      const compressed = await compressProductImage(_scannerPhotoFile);
      const { error: upErr } = await db.storage
        .from('product-images')
        .upload(path, compressed, { contentType: 'image/jpeg', upsert: false });
      if (upErr) {
        setScannerPhotoStatus('Erro no upload: ' + upErr.message, 'error');
        return;
      }
      const { data: { publicUrl } } = db.storage.from('product-images').getPublicUrl(path);
      imageUrl = publicUrl;
    } else {
      imageUrl = _scannerPhotoUrl;
    }

    // Verifica se é admin — admin salva no banco geral, usuário salva override
    const isAdmin = await checkIsAdmin();

    if (isAdmin && scannerProduct.id) {
      // Salva directo na tabela products
      const { error } = await db.from('products')
        .update({ image_url: imageUrl })
        .eq('id', scannerProduct.id);
      if (error) throw error;
    } else {
      // Salva como override pessoal do usuário
      await saveUserOverride(scannerProduct.ean, {
        name: scannerProduct.name,
        brand: scannerProduct.brand || '',
        image_url: imageUrl
      });
    }

    // Actualiza o produto em memória e re-renderiza o card com a nova foto
    scannerProduct = { ...scannerProduct, image_url: imageUrl };
    setScannerPhotoStatus('Foto salva com sucesso!', 'success');
    showToast('Foto adicionada!', 'success');

    // Fecha modal e re-renderiza o card já com a foto
    setTimeout(() => {
      closeScannerPhotoModal();
      renderProductResult(scannerProduct, scannerLastPurchase);
    }, 700);

  } catch (e) {
    console.error('[Scantix] Erro ao salvar foto:', e);
    setScannerPhotoStatus('Erro: ' + e.message, 'error');
  } finally {
    if (btn) { btn.disabled = false; btn.innerHTML = '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M19 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11l5 5v11a2 2 0 0 1-2 2z"/><polyline points="17 21 17 13 7 13 7 21"/><polyline points="7 3 7 8 15 8"/></svg> Salvar foto'; }
  }
}
...
function handleCameraScan(code) {
  ...
  // Se a página de Produtos está em modo de filtro por scanner, delega para ela
  if (window._productsFilterMode && typeof handleProductsCameraScan === 'function') {
    if (handleProductsCameraScan(cleaned)) return;
  }

  // Se o Carrinho está em modo de filtro por scanner, delega para ele
  if (window._cartFilterMode && typeof handleCartCameraScan === 'function') {
    if (handleCartCameraScan(cleaned)) return;
  }

  // Comportamento padrão: popula o input do scanner principal
  const input = document.getElementById('ean-input');
