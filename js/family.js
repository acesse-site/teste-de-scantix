...
async function getOrCreateFamilyGroup() {
  ...
  // Tenta buscar lista (evita erros de .single() com zero rows)
  const { data: list, error: fetchErr } = await db.from('family_groups')
    .select('*').eq('owner_id', user.id).limit(1);

  if (fetchErr) {
    console.error('[Scantix] family_groups fetch error:', fetchErr);
    return null;
  }
  ...
  // Não existe — cria
  const { data: created, error: createErr } = await db.from('family_groups')
    .insert({ owner_id: user.id, name: 'Minha Família' })
    .select().single();

  if (createErr) {
    console.error('[Scantix] family_groups insert error:', createErr);
    return null;
  }
  return created;
}
...
async function acceptInviteCode(code) {
  ...
  // Busca convite pelo código (sem JOIN — RLS bloquearia family_groups para não-donos)
  ...
  // Busca o grupo pelo group_id do convite
  // Precisa de uma policy SELECT em family_groups que permita leitura via invite
  const { data: groupList } = await db.from('family_groups')
    .select('*').eq('id', invite.group_id).limit(1);
  const group = groupList && groupList.length > 0 ? groupList[0] : null;
...
async function getMyMemberships() {
  ...
  const { data } = await db.from('family_members')
    .select('*, family_groups(id, name, owner_id, owner_name, owner_avatar)')
...
async function switchToSharedAccount(membership) {
  ...
  if (!group) { showToast('Erro ao trocar de conta', 'error'); return; }

  const account = {
    type:        'shared',
    ownerId:     group.owner_id,
    ownerName:   group.owner_name || group.name,
    groupId:     group.id,
    permissions: {
...
// ── Actualiza visual dos botões de conta sem re-renderizar tudo ──
function updateAccountButtons() {
  const acc = _activeAccount || loadActiveAccount();
  const btns = document.querySelectorAll('#family-section button[data-account-type]');
  btns.forEach(btn => {
    const isActive = btn.dataset.accountType === acc.type &&
      (acc.type === 'own' || btn.dataset.groupId === acc.groupId);
    btn.style.borderColor = isActive ? 'var(--primary)' : 'var(--border)';
    btn.style.background  = isActive ? 'var(--primary-light)' : 'var(--card)';
    const check = btn.querySelector('.account-check');
...
function updateAccountBanner() {
  // Banner removido — conta activa mostrada apenas no drawer do perfil
...
async function renderFamilySection() {
  ...
  html += `
    <div style="margin-bottom:1rem">
      ...
      <div style="display:flex;flex-direction:column;gap:.375rem">
        <button data-account-type="own" onclick="switchToOwnAccount()" style="
        ...
        ${memberships.map(m => {
          ...
          // Mostra o nome do dono (owner) — usa o nome do grupo que é "Minha Família" do dono
          // Tenta pegar o nome real via user_metadata se disponível, senão usa o nome do grupo
          const ownerDisplayName = g?.owner_name || g?.name || 'Família';
          return `
          <button data-account-type="shared" data-group-id="${m.family_groups?.id}" onclick="switchToSharedAccount(${JSON.stringify(m).replace(/"/g, '&quot;')})" style="
            display:flex;align-items:center;gap:.625rem;padding:.625rem .75rem;border-radius:.625rem;
            border:1.5px solid ${isActive?'var(--primary)':'var(--border)'};
            background:${isActive?'var(--primary-light)':'var(--card)'};
            cursor:pointer;text-align:left;transition:all .15s">
            ${g?.owner_avatar
              ? `<img src="${g.owner_avatar}" style="width:2rem;height:2rem;border-radius:50%;object-fit:cover;flex-shrink:0">`
            ...
            <div style="flex:1;min-width:0">
              <div style="font-size:.8125rem;font-weight:600;color:var(--fg)">${escHtml(ownerDisplayName)}</div>
              <div style="font-size:.7rem;color:var(--muted-fg)">Conta compartilhada</div>
            </div>
  ...
  if (group) {
    ...
    // Grupo ainda não foi criado (provavelmente erro de RLS/tabela) — mostra botão de criar
    html += `
      <div>
        <div style="font-size:.75rem;font-weight:600;color:var(--muted-fg);text-transform:uppercase;letter-spacing:.05em;margin-bottom:.5rem">Meu grupo</div>
        <p style="font-size:.8125rem;color:var(--muted-fg);margin:.5rem 0 .75rem">Crie um grupo para compartilhar sua conta com a família.</p>
        <button onclick="handleCreateGroup()" class="btn-primary" style="width:100%;justify-content:center">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
          Criar meu grupo
        </button>
      </div>`;
...
async function handleCreateGroup() {
  showToast('Criando grupo...', 'info');
  const group = await getOrCreateFamilyGroup();
  if (!group) {
    showToast('Erro ao criar grupo. Verifique o console para detalhes.', 'error');
    return;
  }
