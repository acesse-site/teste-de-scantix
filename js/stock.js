...
async function renderStockPage() {
  ...
  const { data, error } = await db.from('stock').select('*').eq('user_id', activeUserId).order('product_name');
  ...
  return `
    <div class="stock-item ${isZeroed ? 'zeroed' : ''}" data-ean="${item.product_ean}"
      ...
      <div class="stock-actions">
        ${available > 0
          ? `${hasPermission('can_purchase') ? inUseOpenBtn : ''}
             ${hasPermission('can_purchase') ? `<button class="stock-btn-exit" onclick="openStockExitModal('${item.id}')">
          ...
          : hasInUse && hasPermission('can_purchase')
            ? inUseFinishBtn
            : ''
        }
        ${hasPermission('can_purchase') ? `<button class="stock-btn-icon cart" onclick="openStockCartModal('${item.id}')">
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="9" cy="21" r="1"/><circle cx="20" cy="21" r="1"/><path d="M1 1h4l2.68 13.39a2 2 0 0 0 2 1.61h9.72a2 2 0 0 0 2-1.61L23 6H6"/></svg>
        </button>` : ''}
        ${hasPermission('can_delete') ? `<button class="stock-btn-icon trash" onclick="deleteStockItem('${item.id}')">
